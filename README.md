# Array
This repository contains the Array problem solutions.

[0 Reverse array](0_Reverse_the_array.py)

[1 Find max min element array](1_Find_max_min_element_array.py)

[2 Kth smallest element](2_Kth_smallest_element.py)

[3 Sort an array of 0s, 1s and 2s](3_Sort_an_array_of_0s,_1s_and_2s.py)

[4 Move all negative elements to end](4_Move_all_negative_elements_to_end.py)

[5 Union of two arrays](5_Union_of_two_arrays.py)

[6 Cyclically rotate an array by one](6_Cyclically_rotate_an_array_by_one.py)

[7 Kadanes Algorithm](7_Kadanes_Algorithm.py)

[8 Minimize the Heights II](8_Minimize_the_Heights_II.py)

[9 Minimum number of jumps](9_Minimum_number_of_jumps.py)

[10 Find the Duplicate Number](10_Find_the_Duplicate_Number.py)

[11 Merge Without Extra Space](11_Merge_Without_Extra_Space.py)

[12 Reverse array](0_Reverse_the_array.py)

[13 Merge Intervals](13_Merge_Intervals.py)

[14 Next Permutation](14_Next_Permutation.py)

[15 Count Inversions](15_Count_Inversions.py)

[16 Best Time to Buy and Sell Stock](16_Best_Time_to_Buy_and_Sell_Stock.py)

[17 Count pairs with given sum](17_Count_pairs_with_given_sum.py)

[18 find common elements In 3 sorted arrays](18_find_common_elements_In_3_sorted_arrays.py)

[19 Alternate positive and negative numbers](19_Alternate_positive_and_negative_numbers.py)

[20 Subarray with 0 sum](20_Subarray_with_0_sum.py)

[21 Factorials of large_numbers](21_Factorials_of_large_numbers.py)

[22 Maximum Product Subarray](22_Maximum_Product_Subarray.py)

[23 Longest consecutive subsequence](23_Longest_consecutive_subsequence.py)

[24 Majority Element II k n](24_Majority Element_II_k_n.py)

[25 Buy and Sell Stock III](25_Buy_and_Sell_Stock_III.py)

[26 Array Subset of another array](26_Array_Subset_of_another_array.py)

[27 Triplet Sum in Array](27_Triplet_Sum_in_Array.py)

[28 Trapping Rain Water](28_Trapping_Rain_Water.py)

[29 Chocolate Distribution Problem](29_Chocolate_Distribution_Problem.py)

[30 Minimum Size Subarray Sum](30_Minimum_Size_Subarray_Sum.py)

[31 Three way partitioning](31_Three_way_partitioning.py)

[32 Minimum swaps and K together](32_Minimum_swaps_and_K_together.py)

[33 Form a palindrome](33_Form_a_palindrome.py)

[34 Find the median](34_Find_the_median.py)

[35 Median of 2 Sorted Arrays of Different Sizes](35_Median_of_2_Sorted_Arrays_of_Different_Sizes.py)




